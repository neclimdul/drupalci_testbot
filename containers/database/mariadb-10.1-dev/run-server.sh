...
TAG="drupalci/mariadb-10.1"
NAME="drupaltestbot-db-mariadb-10.1"
...
if [[ $RUNNING != "" ]]
  then
    ...
    docker rm $STALLED
    DCI_SQLCONT=(/tmp/tmp.*"mariadb-10.1")
...
TMPDIR=$(mktemp -d --suffix=mariadb-10.1)
...
docker ps | grep "drupalci/mariadb-10.1"
